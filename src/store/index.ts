    ...
    (set, get) => ({
      ...
      addProduct: async (product) => {
        ...
        const { data, error } = await supabase.from('products').insert({
          ...
          image: product.hasNoGarlicOption ? `${product.image.split('#nogarlic')[0]}#nogarlic` : product.image.split('#nogarlic')[0],
        ...
        try {
          ...
          if (productsData) {
            const products = productsData.filter(p => !p.is_combo).map(p => ({
              ...
              category: p.category,
              image: (p.image || '').split('#nogarlic')[0],
              hasNoGarlicOption: (p.image || '').includes('#nogarlic'),
              variants: p.variants,
              inStock: p.in_stock,
